...
#output_cidr
tmp_csv_file = Path(__file__).resolve().parent / "cidr_orgName.csv"
#netranges that don't have organization names
no_orgname_file = Path(__file__).resolve().parent / "no_orgname.csv"
#netranges that don't have organization name and >= /24
outliers_file = Path(__file__).resolve().parent / "outlier.txt"
...
for block in netrange_block:
    ...
    if netrange != "!":
        ...
        if oorgname == "No found":
            for c in cidrs:
                if c.num_addresses >= 256:
                    with open(outliers_file, "a+") as file:
                        file.write(f"The cidr {c} doesn't have orgName\n")
                t = (c, netrange)
